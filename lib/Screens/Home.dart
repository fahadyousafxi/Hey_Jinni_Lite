...
Widget build(BuildContext context) {
  return WillPopScope(
    onWillPop: () async {
      bool? goBack = await _webViewController.future
          .then((value) => value.canGoBack());
      if (goBack != true) {
        return true;
      } else {
        _webViewController.future
            .then((controller) => controller.goBack());
        return false;
      }
    },
    child: Scaffold(
              ...
              children: [
                WebView(
                  ...
                  onPageStarted: (String url) {
                    ...
                    setState(() {
                      _isLoading = true; // Set loading state to true when a new page starts loading
                    });
                  },
                  onPageFinished: (String url) {
                    print('Page finished loading: $url');
                    setState(() {
                      _isLoading = false; // Set loading state to false when the page finishes loading
                    });
                  },
                ...
                Visibility(
                  visible: _isLoading, // Show the progress indicator only when loading
                  child:Center(
                    child: CircularPercentIndicator(
                          radius: 80.0,
                          lineWidth: 15.0,
                          percent: _progress,
                          center: new Text("$_progressText%",style: TextStyle(color: MyColors.kprimaryColor,fontSize: 40),),
